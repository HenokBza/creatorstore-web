...
export default function PhonePage() {
  ...
  const [countryIndex, setCountryIndex] = useState(2); // Default to Ethiopia if you want, or 0
  ...
  const handleContinue = async () => {
    ...
    if (!phone.startsWith(selected.code)) {
      setError(`Phone number must start with ${selected.code}`);
      return;
    }
    ...
    try {
      // IF ETHIOPIA: Check Firestore for payment verification whitelist
      if (selected.name === "Ethiopia") {
        const q = query(
          collection(db, "pendingApplications"),
          where("phone", "==", phone.trim())
        );
        const querySnapshot = await getDocs(q);

        if (querySnapshot.empty) {
          // No record found at all -> redirect to registration/payment page
      ...
      // FOR OTHER COUNTRIES OR APPROVED ETHIOPIAN USERS: Save and proceed
      localStorage.setItem("signupCountry", selected.name);
      localStorage.setItem("signupPhone", phone.trim());
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Select your country and enter your phone number before creating your account.
        </p>
        ...
        >
          ...
          <div>
            ...
            >
              Example: {selected.code}XXXXXXXXX
            </p>
          </div>
